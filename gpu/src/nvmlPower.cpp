...
*/
const char* ExtractBaseFilename(const char* filePath) {
...
void *powerPollingFunc(void *ptr)
{
    ...
    // Convert the base filename to a char*
    const char* baseFilenameChar = ExtractBaseFilename((const char*)origfilepath);
	char newFileName[256]; // Adjust the size as needed
    snprintf(newFileName, sizeof(newFileName), "./power_logs/%s.log", baseFilenameChar);
...
{
	int i;
	strcpy(origfilepath, (const char*)filename);
	// Initialize nvml.
	nvmlResult = nvmlInit();
